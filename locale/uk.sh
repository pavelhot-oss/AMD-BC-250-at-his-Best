...
⚠️  Бінарний файл зібрано. Дотримуйтеся README репозиторію для точного
    встановлення (сервіс systemd, шляхи), яке може змінюватися з часом:
        %s/README.md
    Коли бінарний файл/сервіс буде встановлено самим репозиторієм,
    поверніться сюди для налаштування.
"
MSG[m06_installed_q]="Регулятор встановлено (бінарний файл + сервіс systemd наявні)? Продовжити з налаштуванням?"
MSG[m06_config_postponed]="Налаштування відкладено. Запустіть цей модуль знову, коли встановлення регулятора буде завершено."
...
MSG[m06_toml_no_volt]="# напругу не задано в конфігурації -> перевірте типове значення регулятора"
MSG[m06_file_written]="Файл записано: %s"
MSG[m06_progression]="
Прогресія, рекомендована Old Lamer:
  1500 МГц (стандарт) -> 2000 МГц (легкий крок, ~10%%+ у FurMark)
  -> перевірте CPU на 3.85 ГГц (модуль 05) -> підіймайте GPU далі
  ЛИШЕ якщо охолодження справляється (водяне: повідомляють до ~2.4 ГГц,
  ~360 Вт / 30 А, звідси й важливість додаткових роз'ємів Molex
  з модуля 01).

⚠️  ВАЖЛИВО: перевірте цей файл конфігурації ВРУЧНУ (запустіть сервіс
    у передньому плані / вручну), перш ніж вмикати його при завантаженні.
    НЕ вмикайте сервіс автоматично з цього скрипта."
MSG[m06_restart_q]="Перезапустити сервіс регулятора зараз, щоб застосувати перевірену конфігурацію?"
MSG[m06_service_name_hint]="Точна назва сервісу може відрізнятися залежно від версії репозиторію — перевірте через 'systemctl list-units | grep -i cyan'."
MSG[m06_enable_yourself]="Коли стабільність підтверджено вручну, увімкніть збереження самостійно за допомогою:"
MSG[m06_upstream_service]="Upstream-репозиторій (гілка smu) надає власний сервіс systemd."
MSG[m06_see_readme]="Дивіться %s/README.md щодо точної процедури встановлення."
...
MSG[m09_freq_target]="%s МГц (ціль: %s МГц)"
...
MSG[m09_dry_services]="[DRY-RUN] Перевірка сервісів systemd (bc250-core-unlock, bc250-smu-oc, cyan-skillfish-governor)"
...
MSG[m09_dry_gpu_freq]="[DRY-RUN] Перевірка частоти GPU через /sys/class/drm/card0/device/pp_dpm_sclk або rocm-smi (ціль: %s МГц ±100 МГц)"
MSG[m09_t_gpu_freq_near]="Частота GPU (GPU_FREQ_MHZ ±100 МГц)"
MSG[m09_gpu_freq_msg]="%s МГц (ціль: %s МГц, джерело: %s)"
MSG[m09_t_gpu_freq_diff]="Частота GPU (відхилення > 100 МГц)"
MSG[m09_gpu_freq_diff_msg]="%s МГц (ціль: %s МГц, відхилення: %s МГц, джерело: %s)"
