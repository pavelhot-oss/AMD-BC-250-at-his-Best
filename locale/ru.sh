...
MSG[m05_detect_failed]="bc250-detect не сработал - возврат к стоковым значениям."
MSG[m05_config_missing]="Стабильная конфигурация не создана (overclock.conf отсутствует) - возврат к стоковым значениям."
...
MSG[m05_apply_install_failed]="Не удалось записать конфигурацию загрузки (bc250-apply --install не сработал)."
MSG[m05_service_enabled]="Служба bc250-smu-oc включена при загрузке с %s МГц @ %s мВ."
MSG[m05_service_missing]="bc250-smu-oc.service не создан - разгон не будет применяться при загрузке."
MSG[m05_service_enable_failed]="Не удалось включить bc250-smu-oc.service при загрузке."
...
MSG[m06_follow_readme]="Установка из %s:
    бинарный файл, политика D-Bus, обёртка performance-mode и
    служба systemd cyan-skillfish-governor-smu.
Служба запускается сразу (перезагрузка не требуется)."
MSG[m06_installed_q]="Установить регулятор (бинарный файл + служба systemd) и применить настройку ниже?"
MSG[m06_config_postponed]="Установка пропущена. Запустите этот модуль снова, чтобы установить и запустить регулятор."
...
MSG[m06_progression]="Рекомендуемая Old Lamer прогрессия:
  1500 МГц (сток) -> 2000 МГц (лёгкий шаг, ~10%%+ в FurMark)
  -> проверить CPU на 3,85 ГГц (модуль 05) -> разгонять GPU дальше
  ТОЛЬКО если охлаждение успевает (водянка: сообщают до ~2,4 ГГц,
  ~360 Вт / 30 А, отсюда важность дополнительных разъёмов Molex
  из модуля 01).

  ВАЖНО: после запуска службы дайте реальную нагрузку на GPU
  (FurMark Vulkan + игра) несколько минут и проверьте логи
  (journalctl -u cyan-skillfish-governor-smu) перед тем, как доверять."

# ------------------------------------------------------------------
# Module 07 - system tuning
# ------------------------------------------------------------------
MSG[m06_bin_installed]="Бинарный файл установлен: %s"
MSG[m06_perf_installed]="Обёртка performance-mode установлена: %s"
MSG[m06_dbus_installed]="Политика D-Bus установлена: %s"
MSG[m06_service_installed]="Служба systemd установлена: %s"
MSG[m06_started]="Служба регулятора активна."
MSG[m06_start_failed]="Служба регулятора запустилась с ошибками. Проверьте службу:"
MSG[m06_enable_boot_q]="Включить службу регулятора при загрузке (повторно применяет разгон при каждом старте)?"
MSG[m06_enabled]="Служба включена при загрузке."
...
MSG[m07_zswap_zram_active]="zram уже обеспечивает сжатый swap на этой системе (%s) - zswap пропущен (запуск обоих только добавит накладные расходы из-за двойного сжатия)."
...
MSG[m09_threads_bios_unlocked]="потоков: %s - разблокировка 8 ядер активна через модифицированный BIOS (модуль 02)"
MSG[m09_threads_bios_not_enabled]="потоков: %s - включите «Unlock CPU cores» в модифицированном BIOS"
...
MSG[m09_freq_target]="%s МГц под нагрузкой (цель: %s МГц)"
...
MSG[m09_freq_under_target]="%s МГц под нагрузкой (цель: %s МГц, отклонение: %s МГц) - OC не применён (модуль 05) или троттлинг"
...
MSG[m09_dry_services]="[DRY-RUN] Проверка служб systemd (bc250-core-unlock.service, bc250-smu-oc.service, cyan-skillfish-governor-smu.service)"
...
MSG[m09_svc_bios_governs]="не требуется - ядра управляются модифицированным BIOS (модуль 02)"
...
MSG[m09_gpu_freq_msg]="%s МГц верхнее состояние (цель: %s МГц, текущее: %s МГц)"
MSG[m09_t_gpu_freq_diff]="Частота GPU (отклонение > 100 МГц)"
MSG[m09_gpu_freq_diff_msg]="%s МГц верхнее состояние (цель: %s МГц, отклонение: %s МГц, текущее: %s МГц)"
...
MSG[m09_gpu_furmark_vk]="FurMark 2.x: стресс Vulkan через демо furmark-vk (то, что MangoHud умеет подсвечивать - во 2.x нет -t)"
MSG[m09_furmark_v2_demo]="FurMark 2.x: флага -t больше нет, вместо него запускается Vulkan-демо furmark-vk"
MSG[m09_furmark_missing]="FurMark не установлен, тест GPU пропущен"
MSG[m09_gpu_pair_mangohud]="Стресс GPU в паре с MangoHud (живой оверлей sclk/темп/VRAM во время теста - MangoHud ставит модуль 07). Best-effort: если FurMark грузит чистый OpenGL, оверлей не покажется, но стресс всё равно идёт."
MSG[m09_no_graphical_session]="Графическая сессия с клиентом отображения не найдена (headless/SSH?). Тест стресса FurMark пропущен - система считается валидной."
