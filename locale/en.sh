...
# ------------------------------------------------------------------
MSG[i18n_invalid]="Unknown language: %s (expected: en|fr)"
MSG[i18n_switched]="Language: %s"
...
Usage:
  ...
  sudo ./install.sh --lang en       # UI language: en | fr (also BC250_LANG
                                    # env var, or UI_LANG in the config file)
...
MSG[inst_lang_needs_arg]="--lang requires a language code (en|fr)"
...
⚠️  The binary is built. Follow the repository README for the exact
    installation (systemd service, paths), which may change over time:
        %s/README.md
    Once the binary/service are installed by the repository itself, come
    back here for the configuration.
"
MSG[m06_installed_q]="Is the governor installed (binary + systemd service present)? Continue with the configuration?"
MSG[m06_config_postponed]="Configuration postponed. Re-run this module once the governor installation is complete."
...
MSG[m06_toml_no_volt]="# voltage not set in the config -> check the governor's default value"
MSG[m06_file_written]="File written: %s"
MSG[m06_progression]="
Progression recommended by Old Lamer:
...
⚠️  IMPORTANT: test this config file MANUALLY (start the service in the
    foreground / by hand) before enabling it at boot.
    Do NOT enable the service automatically from this script."
MSG[m06_restart_q]="Restart the governor service now to apply the tested config?"
MSG[m06_service_name_hint]="The exact service name may differ depending on the repo version — check with 'systemctl list-units | grep -i cyan'."
MSG[m06_enable_yourself]="Once stability is confirmed manually, enable persistence yourself with:"
MSG[m06_upstream_service]="The upstream repo (smu branch) provides its own systemd service."
MSG[m06_see_readme]="See %s/README.md for the exact installation procedure."
...
MSG[m09_freq_target]="%s MHz (target: %s MHz)"
...
MSG[m09_dry_services]="[DRY-RUN] Checking the systemd services (bc250-core-unlock, bc250-smu-oc, cyan-skillfish-governor)"
...
MSG[m09_dry_gpu_freq]="[DRY-RUN] Checking the GPU frequency via /sys/class/drm/card0/device/pp_dpm_sclk or rocm-smi (target: %s MHz ±100 MHz)"
MSG[m09_t_gpu_freq_near]="GPU frequency (close to GPU_FREQ_MHZ ±100 MHz)"
MSG[m09_gpu_freq_msg]="%s MHz (target: %s MHz, source: %s)"
MSG[m09_t_gpu_freq_diff]="GPU frequency (deviation > 100 MHz vs config)"
MSG[m09_gpu_freq_diff_msg]="%s MHz (target: %s MHz, deviation: %s MHz, source: %s)"
